...
interface OnboardingLunchTimeSetProps {
  onNext?: () => void;
}

export default function OnboardingLunchTimeSet({ onNext }: OnboardingLunchTimeSetProps) {
  const { width } = useWindowDimensions();
  const isTablet = width > 600;
  const MAX_WIDTH = isTablet ? 420 : 360;

  const [selectedTime, setSelectedTime] = useState<number | null>(11); // Figma 디자인에서 11시가 선택된 상태
  ...
  const handleNext = () => {
    if (isNextButtonActive) {
      console.log('선택된 점심 약 시간:', selectedTime);
      onNext?.();
    }
  };
