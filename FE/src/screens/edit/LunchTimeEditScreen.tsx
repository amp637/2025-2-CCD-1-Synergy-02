...
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'LunchTimeEdit'>;

const timeOptions = [11, 12, 1, 2, 3, 4];

export default function LunchTimeEditScreen() {
  ...
  // 기존 점심 복약 시간 불러오기
  const loadLunchTime = async () => {
    try {
      // 실제 API 호출로 대체
      // GET /api/v1/users/me/medication-times?type=lunch


      // 예시 데이터 (12시)
      setSelectedTime(12);
    } catch (error) {
      console.error('점심 복약 시간 불러오기 실패:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleNext = () => {
    if (isNextButtonActive) {
      // 선택된 점심 복약 시간을 저장하거나 전달
      navigation.navigate('EveningTimeEdit');
    }
  };
